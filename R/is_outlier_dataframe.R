###############################################################################
# is_outlier_dataframe
###############################################################################
#
# function returns a logical vector indicating whether a row is an outlier
#
...
# between the row and the column medians is computed and passed to `is_outlier`
#
###############################################################################
#
...
is_outlier_dataframe <- function(x, coef = 1.5){
  ...
  return( is_outlier(pointEst, coef = coef) )
}
