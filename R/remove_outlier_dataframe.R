###############################################################################
# remove_outlier_dataframe
###############################################################################
#
# function returns a dataframe after removing the outliers
#
...
# between the row and the column medians is computed and passed to
# `remove_outlier`
#
###############################################################################
#
...
remove_outlier_dataframe <- function(x, coef = 1.5){
  ...
  return( x[!is_outlier(pointEst, coef = coef),] )
}
